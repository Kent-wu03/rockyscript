...
-- @name    SBLM Pabrik
-- @author  RockyBandel
-- @desc    Auto farm pabrik: PNB, plant, harvest, auto trash, auto vend seed
---CONFIG---
-- @field idb      number  880             ID Block
-- @field bhit     number  2               Jumlah Hit Block
-- @field plat     number  102             ID Platform/Plat Farm
-- @field wpabrik  string  worldkalian     Nama World Pabrik
-- @field idpabrik string  iddoor          ID Door World Pabrik
-- @field trash    idlist  5026,5024,5028  ID Trash (pisah koma)
-- @field delay_recon number 3500          Delay Reconnect (ms)
---END---

-- ================================================
--   SBLM MAIN - jangan edit, config via Launcher
-- ================================================
...
local function ltc(text) LogToConsole("`^" .. text) end
...
local function getTile(x, y)
    for _, tile in pairs(GetTiles()) do
        if tile.x == x and tile.y == y then return tile end
    end
    return nil
end
...
local function collectDrop()
    ...
    for _, obj in pairs(GetObjectList()) do
        ...
        if math.abs(ox - bx) <= 2 and math.abs(oy - config.ey) <= 2 then
            collect(obj); Sleep(20)
        end
    end
end
...
local function plant(x, y)
    SendPacketRaw(false, {type=3, value=config.ids, px=x, py=y, x=GetLocal().pos.x, y=GetLocal().pos.y})
    Sleep(180)
end

local function placeb(x, y)
    SendPacketRaw(false, {type=3, value=config.idb, px=x, py=y, x=GetLocal().pos.x, y=GetLocal().pos.y})
    Sleep(180)
end
...
local function checktree()
    for _, p in pairs(pos) do
        ...
        if tile and tile.fg == config.ids then
            if not tile.extra or tile.extra.progress < 1.0 then return false end
        end
    end
    return true
end

local function addvend()
    AddHook('OnVariant', 'blockvending', function(var)
        if var[0] == 'OnDialogRequest' then return true end
    end)
    log("Add seed ke vend")
...
local function pnb()
    ...
    while inv(config.idb) > 10 do
        collectDrop()
        placeb(config.ex - 1, config.ey)
        for i = 1, config.bhit do breakb(config.ex - 1, config.ey) end
    end
end
...
local function ht()
    while not checktree() do Sleep(5000) end
    log("Harvesting")
    for _, p in pairs(pos) do
        local above = getTile(p.x, p.y - 1)
...
local function pabrik()
    ...
    if not world or world.name ~= config.wpabrik then
        ...
        world = GetWorld()
        if world and world.name == config.wpabrik then dc = true end
    end
...
RunThread(function()
    while true do
        ...
        if not ok then ltc("Error: "..tostring(err)); Sleep(3000) end
        Sleep(200)
    end
end)
